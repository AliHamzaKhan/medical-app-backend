...
def get_current_active_superuser_or_doctor(current_user: models.User = Depends(get_current_user)) -> models.User:
    if not (crud.user.is_superuser(current_user) or crud.user.is_doctor(current_user)):
        raise HTTPException(
